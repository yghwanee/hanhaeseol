...
async function main() {
  const failureCaption = "❌ 인스타 게시 실패. 아래 카드를 수동으로 업로드해주세요.";
  const outDir = path.resolve("generated/instagram");
  const manifestPath = path.join(outDir, "manifest.json");

  if (!fs.existsSync(manifestPath)) {
    await sendText("❌ 인스타 카드 생성/게시 실패. GitHub Actions 로그 확인.");
    return;
  }
  ...
  const items: MediaItem[] = manifest.files.map((filename, i) => ({
    buf: fs.readFileSync(path.join(outDir, filename)),
    filename,
    caption: i === 0 ? failureCaption : undefined,
  }));
