...
const SLOTS = {
  ...
  inline: {
    // 🔴 PC 도 300x250 모바일 유닛을 쓴다(2026-08-14). 원래 PC 는 728x90
    // `DAN-lcALm7uz8M1Y77F7` 이었는데 **애드핏 서버가 그 유닛에만 `NO_AD` 를 돌려줘**
    // 자리가 종일 비어 있었다. 대시보드상 `승인`·유형 728x90 정상이고, 마크업·SDK·요청
    // 파라미터 27개를 공식 형태로 맞춰도 유닛 ID 만 바꾸면 결과가 갈렸다(작업90).
    // 이 300x250 유닛은 **PC 폭에서도 정상으로 채워지는 것을 실측**해 임시 대체한다.
    ...
    // 이제 PC 도 300 폭이라 본문 컬럼(실효 640) 안에 들어간다 — breakout 불필요.
