// 네이버 스포츠 API의 팀명 → schedule.json의 팀명 매핑.
// 키: 네이버 응답의 teamName / 값: schedule.json에서 같은 팀을 가리키는 한국어 표기.
// 한 팀이 schedule에서 여러 표기로 등장하면 string[]로 다 적는다 (예: 샌디에이고 FC, 샌디에이고).
// 네이버 원본 이름은 fetcher가 자동으로 키에 추가하므로 여기엔 적지 않아도 매칭됨.
/**
 * 유럽 클럽 공통 표기. 컵·프리시즌 친선처럼 여러 리그 팀이 한 카테고리에 섞이는 대회는
 * 리그별 표를 따로 쓸 수 없어 이 묶음을 그대로 재사용한다.
 * (값은 리그별 표에서 이미 실전 검증된 편성표 표기만 옮겨왔다 — 새로 지어내지 않는다.)
 */
const EURO_CLUB_ALIASES: Record<string, string | string[]> = {
  "맨체스터 시티": "맨시티",
  "맨체스터 유나이티드": "맨유",
  "토트넘 홋스퍼": "토트넘",
  "아스널": "아스날",
  "뉴캐슬 유나이티드": "뉴캐슬",
  "리즈 유나이티드": "리즈",
  "노팅엄 포레스트": "노팅엄",
  "애스턴 빌라": "아스톤 빌라",
  "브라이턴 앤 호브": "브라이튼",
  "브렌트퍼드": "브렌트포드",
  "웨스트햄 유나이티드": "웨스트햄",
  "크리스털 팰리스": "크리스탈 팰리스",
  "울버햄튼 원더러스": "울버햄튼",
  "보루시아 도르트문트": "도르트문트",
  "바이엘 레버쿠젠": "레버쿠젠",
  "인테르 밀라노": "인터 밀란",
  "인테르나치오날레": "인터 밀란",
  // 편성 표기가 제공처마다 갈린다: 라리가 편성은 "AT. 마드리드"(점+공백),
  // 예전 표기는 "AT.마드리드"(공백 없음). 둘 다 적어 둔다.
  "아틀레티코 마드리드": ["AT. 마드리드", "AT.마드리드"],
  "아틀레틱 빌바오": "아틀레틱",
  "올림피크 마르세유": "마르세유",
  "올랭피크 리옹": "리옹",
  PSG: "파리 생제르망",
  "파리 생제르맹": "파리 생제르망",
  "클럽 브뤼헤": "클뤼프 브뤼헤",
  "사우샘프턴": "사우스햄튼",
};

export const NAVER_TO_SCHEDULE_TEAM_NAME: Record<string, Record<string, string | string[]>> = {
  KBO: {},

  MLB: {
    // 편성은 "LA 다저스" 와 "로스앤젤레스 다저스" 가 공존한다(2026-09-15 전적 감사).
    "LA다저스": ["LA 다저스", "로스앤젤레스 다저스"],
    "LA에인절스": "LA 에인절스",
    "뉴욕메츠": "뉴욕 메츠",
    "뉴욕양키스": "뉴욕 양키스",
    "디트로이트": "디트로이트 타이거스",
    "마이애미": "마이애미 말린스",
    "미네소타": "미네소타 트윈스",
    "밀워키": "밀워키 브루어스",
    "보스턴": "보스턴 레드삭스",
    "볼티모어": "볼티모어 오리올스",
    "샌디에이고": "샌디에이고 파드리스",
    "샌프란시스코": "샌프란시스코 자이언츠",
    "세인트루이스": "세인트루이스 카디널스",
    "시애틀": "시애틀 매리너스",
    "시카고W": "시카고 화이트삭스",
    "시카고컵스": "시카고 컵스",
    "신시내티": "신시내티 레즈",
    "애리조나": "애리조나 다이아몬드백스",
    "애슬레틱스": "애슬레틱스",
    "애틀랜타": "애틀랜타 브레이브스",
    "워싱턴": "워싱턴 내셔널스",
    "캔자스시티": "캔자스시티 로열스",
    "콜로라도": "콜로라도 로키스",
    "클리블랜드": "클리블랜드 가디언스",
    "탬파베이": "탬파베이 레이스",
    "텍사스": "텍사스 레인저스",
    "토론토": "토론토 블루제이스",
    "피츠버그": "피츠버그 파이리츠",
    "필라델피아": "필라델피아 필리스",
    "휴스턴": "휴스턴 애스트로스",
  },

  프리미어리그: {
    // 순위표(네이버)는 풀네임, 편성은 축약 — 로고가 안 붙던 표기(2026-09-03 감사).
    "입스위치 타운": "입스위치",
    "노팅엄 포레스트": "노팅엄",
    "뉴캐슬 유나이티드": "뉴캐슬",
    "리즈 유나이티드": "리즈",
    "맨체스터 시티": "맨시티",
    "맨체스터 유나이티드": "맨유",
    "브라이턴 앤 호브": "브라이튼",
    "브렌트퍼드": "브렌트포드",
    "아스널": "아스날",
    "애스턴 빌라": "아스톤 빌라",
    "울버햄튼 원더러스": "울버햄튼",
    "웨스트햄 유나이티드": "웨스트햄",
    "크리스털 팰리스": "크리스탈 팰리스",
    // 2026년경부터 네이버 응답이 짧은 표기로 옴
    "팰리스": "크리스탈 팰리스",
    "토트넘 홋스퍼": "토트넘",
    // 2026-09-15 전적 감사. 네이버 "코벤트리 시티" / 편성 "코번트리".
    "코벤트리 시티": "코번트리",
  },

  라리가: {
    // 네이버 순위표 표기가 "데포르티보 라 코루냐 A" 다(A 는 네이버가 붙인 꼬리표).
    "데포르티보 라 코루냐 A": "데포르티보",
    "아틀레티코 마드리드": "AT.마드리드",
    "아틀레틱 빌바오": "아틀레틱",
    "셀타 데 비고": "셀타 비고",
    "바예카노": "라요 바예카노",
    "오비에도": "레알 오비에도",
    // 2026년경부터 네이버 응답이 다음 표기로 옴
    // 🔴 값이 "AT.마드리드"(공백 없음) 하나뿐이라 편성의 "AT. 마드리드"(점+공백)와
    // 안 맞았다. resultKey 는 정규화를 안 하므로 표기가 정확히 같아야 매칭된다
    // (2026-08-19 감사에서 라리가 MISS 1건으로 잡힘).
    "AT 마드리드": ["AT. 마드리드", "AT.마드리드"],
    "AT.마드리드": "AT. 마드리드",
    "베티스": "레알 베티스",
    "빌바오": "아틀레틱",
    "소시에다드": "레알 소시에다드",
  },

  세리에A: {
    "프로시노네": "프로시노네 칼초",
    "로마": "AS 로마",
    "밀란": "AC 밀란",
    "베로나": "헬라스 베로나",
    "사수올로": "사수올로 칼초",
    "우디네세": "우디네세 칼초",
    "인테르 밀라노": "인터 밀란",
    // 2026-09-15 감사: 네이버가 "밀라노" 를 떼고 "인테르" 로만 준다. 인터-우디네세 5-3 이 안 붙었다.
    "인테르": "인터 밀란",
    // 🔴 같은 네이버라도 순위·전적 API 는 "인테르나치오날레" 로 준다(결과 API 와 표기가 다르다).
    // 이게 없어 인터 밀란 카드에만 최근 5경기 뱃지가 안 떴다(2026-09-15 audit:aliases 전적 감사).
    "인테르나치오날레": "인터 밀란",
    "칼리아리": "칼리아리 칼초",
    "코모": "코모 1907",
    "파르마": "파르마 칼초",
  },

  분데스리가: {
    // 창단연도 접미사 유무 차이. 편성은 떼고 온다(2026-09-03 감사).
    "파더보른 07": "파더보른",
    "샬케 04": "샬케",
    "FC 쾰른": "쾰른",
    "RB 라이프치히": "라이프치히",
    "TSG 호펜하임": "호펜하임",
    "마인츠 05": "마인츠",
    "바이엘 레버쿠젠": "레버쿠젠",
    "보루시아 글라트바흐": "묀헨글라트바흐",
    "보루시아 도르트문트": "도르트문트",
    "상 파울리": "장크트파울리",
    "함부르크 SV": "함부르크",
  },

  "리그 1": {
    "올랭피크 리옹": "리옹",
    "올림피크 마르세유": "마르세유",
    "파리 생제르맹": "PSG",
    "렌": "스타드 렌",
  },

  MLS: {
    // 네이버 "애틀란타"(란) / 편성 "애틀랜타"(랜). 한 글자 차이로 로고가 안 붙었다.
    "애틀란타 유나이티드": "애틀랜타 유나이티드",
    // 위의 "솔트 레이크" 항목과 별개로, 네이버 순위표 표기는 "리얼 솔트 레이크" 다.
    "리얼 솔트 레이크": ["솔트레이크", "레알 솔트레이크"],
    // 네이버는 짧게 "LAFC"로 옴. Apple TV+는 "로스앤젤레스 FC"로,
    // 쿠팡플레이는 "LAFC"로 와서 schedule에 두 표기가 공존한다.
    "LAFC": "로스앤젤레스 FC",
    "로스앤젤레스 FC": "LAFC",
    "샌디에고 FC": ["샌디에이고 FC", "샌디에이고"],
    "세인트루이스 시티": ["세인트루이스 시티 FC", "세인트루이스"],
    "올랜도 시티": "올랜도 시티 SC",
    // 2026-08-19 개막 후 감사에서 잡힌 축약 표기. 네이버가 도시명만 주는 팀들이다.
    // 각 건 모두 감사가 "같은 경기"임을 확인한 것만 넣었다(추측 없음).
    "갤럭시": "LA 갤럭시",
    // 네이버는 NY Red Bulls 를 "뉴욕", NYCFC 를 "뉴욕 시티 FC" 로 구분해서 준다.
    "뉴욕": "레드불 뉴욕",
    "휴스턴": "휴스턴 다이너모 FC",
    "인터 마이애미": "인터 마이애미 CF",
    // 네이버는 "솔트 레이크"(공백)로 옴. 쿠팡플레이="솔트레이크", Apple TV+="레알 솔트레이크"
    "솔트 레이크": ["레알 솔트레이크", "솔트레이크"],
    "콜롬버스 크루": "콜럼버스 크루",
    "스포팅 KC": "스포팅 캔자스시티",
    // 네이버는 짧게 "화이트캡스"·"시카고"로 옴
    "화이트캡스": "밴쿠버 화이트캡스 FC",
    "시카고": "시카고 파이어 FC",
    "휴스턴 디나모": ["휴스턴", "휴스턴 다이너모 FC"],
    "내쉬빌 SC": "내슈빌 SC",
    "시애틀 사운더스": "시애틀 사운더스 FC",
    // 편성에 "산호세" 로만 오는 제공처가 있다(2026-09-15 전적 감사).
    "SJ 어스퀘이크": ["산호세 어스퀘이크스", "산호세"],
    "뉴욕 레드 불스": "레드불 뉴욕",
    // 2026년경 네이버 응답이 더 짧은 표기로 바뀐 변형들
    "샬럿": "샬럿 FC",
    "NYCFC": "뉴욕 시티 FC",
    // 쿠팡플레이는 SC 없이 "내슈빌"로, Apple TV+는 "내슈빌 SC"로 옴
    "내쉬빌": ["내슈빌 SC", "내슈빌"],
    "신시내티": "FC 신시내티",
    "마이애미": "인터 마이애미 CF",
    "시애틀": "시애틀 사운더스 FC",
    "어스퀘이크": "산호세 어스퀘이크스",
    "뉴잉글랜드": "뉴잉글랜드 레볼루션",
  },

  // 코리아컵(FA컵). 시민구단은 네이버가 "당진", 편성은 "당진시민" 처럼 갈린다.
  코리아컵: {
    당진: "당진시민",
  },

  K리그: {},
  K리그2: {},

  챔피언스리그: {
    "맨체스터 시티": "맨시티",
    "맨체스터 유나이티드": "맨유",
    "토트넘 홋스퍼": "토트넘",
    "아스널": "아스날",
    "뉴캐슬 유나이티드": "뉴캐슬",
    "보루시아 도르트문트": "도르트문트",
    "바이엘 레버쿠젠": "레버쿠젠",
    "인테르 밀라노": "인터 밀란",
    "인테르나치오날레": "인터 밀란",
    "아틀레티코 마드리드": "AT.마드리드",
    "아틀레틱 빌바오": "아틀레틱",
    "올림피크 마르세유": "마르세유",
    // 네이버는 PSG로 응답 (풀네임 "파리 생제르맹"도 혹시 모를 케이스 대비 유지)
    PSG: "파리 생제르망",
    "파리 생제르맹": "파리 생제르망",
    "클럽 브뤼헤": "클뤼프 브뤼헤",
  },

  유로파리그: {
    // SPOTV NOW 스코티시 편성은 "FC" 를 붙여 온다.
    "셀틱": "셀틱 FC",
    "노팅엄 포레스트": "노팅엄",
    "애스턴 빌라": "아스톤 빌라",
    "셀타 데 비고": "셀타 비고",
    "올랭피크 리옹": "리옹",
    "포르투": "FC 포르투",
    "스포르팅 브라가": "브라가",
    "프라이부르크": "SC 프라이부르크",
    // 2026-09-15 전적 감사(audit:aliases)로 잡은 9팀. 편성(SPOTV NOW·쿠팡)과 네이버 전적 표기가 갈린다.
    "아라랏": "아라라트 아르메니아",
    "오모니아 니코시아": "AC 오모니아",
    "바이엘 레버쿠젠": "레버쿠젠",
    "NK 켈예": "NK 첼레",
    "선덜랜드": "선덜랜드 AFC",
    "알크마르": "AZ 알크마르",
    OFI: "OFI 크레테",
    "NEC 네이메헌": "네이메헌",
    "베식타시": "베식타스",
  },

  에레디비시: {
    // 네이버 "아인트호벤" / 편성 "에인트호번" — 같은 PSV 다.
    "아인트호벤": "에인트호번",
    "포르투나 시타르트": "포르튀나",
    "PEC 즈볼레": "즈볼러",
    // 2026-09-15 감사. 네이버 "빌럼 II" / 편성(쿠팡플레이) "빌럼".
    "빌럼 II": "빌럼",
  },

  KBL: {
    // schedule.json은 짧은 표기, 네이버 응답은 도시명 포함 풀 표기
    "고양 소노": "소노",
    "부산 KCC": "KCC",
  },

  "EFL 챔피언십": {
    "사우샘프턴": "사우스햄튼",
  },
  "잉글랜드 FA컵": {},
  "코파 이탈리아": {},

  // 프리시즌 클럽 친선·단판 슈퍼컵 — 여러 리그 클럽이 한 카테고리에 섞인다.
  "클럽 친선경기": { ...EURO_CLUB_ALIASES },
  "FA 커뮤니티 실드": { ...EURO_CLUB_ALIASES },
  "UEFA 슈퍼컵": { ...EURO_CLUB_ALIASES },
  // 독일 컵·슈퍼컵도 여러 리그(1·2·3부, 지역리그) 팀이 한 카테고리에 섞인다.
  "DFB-포칼": { ...EURO_CLUB_ALIASES },
  "프란츠 베켄바워 슈퍼컵": { ...EURO_CLUB_ALIASES },
  // 스코티시 프리미어십. 네이버는 접미사 없이 주고(애버딘·레인저스) SPOTV NOW 편성은
  // "FC" 를 붙인다. 🔴 감사가 "같은 경기"로 확인해 준 것만 넣는다 — 나머지 구단(셀틱·하츠·
  // 킬마녹 등)은 우리 편성에 아직 안 들어왔고, 들어오면 audit:aliases 가 잡는다.
  "스코티시 프리미어십": {
    애버딘: "애버딘 FC",
    레인저스: "레인저스 FC",
    // 2026-09-03 audit:aliases 가 MISS 로 잡은 2건. 예고대로 편성에 들어오면서 드러났다.
    // 네이버는 "팰커크"(패), 편성(SPOTV NOW)은 "폴커크"(포) — 표기가 갈린다.
    팰커크: "폴커크",
    하이버니언: "하이버니언 FC",
    셀틱: "셀틱 FC",
    "세인트 미렌": "세인트 미렌 FC",
    "세인트 존스톤": "세인트 존스톤 FC",
  },

  // 덴마크 1부 Superliga. schedule.json엔 아직 매치 없지만 미래 대비.
  // 들어올 때 league 표기는 "수페르리가" 가정.
  수페르리가: {},
  // ACL 엘리트. 편성(쿠팡플레이)은 짧게·다르게 부른다. 2026-09-15 audit:aliases 가
  // "같은 경기"로 짝지어 준 5건만 넣었다. 🔴 "알 아흘리" 키는 정확히 그 표기에만 걸린다 —
  // 네이버의 "샤바브 알 아흘리" 는 별개 팀이고 여기 영향 없다.
  ACL: {
    "알 아흘리": "알 아흘리 SFC",
    CAHN: "꽁안 하노이",
    "다룰 탁짐": "조호르",
    "가시와 레이솔": "가시와",
    "비셀 고베": "고베",
  },

  NBA: {
    "LA레이커스": "LA 레이커스",
    "LA클리퍼스": "LA 클리퍼스",
    "뉴올리언즈": "뉴올리언스",
    "오클라호마 시티": "오클라호마시티",
  },

  V리그: {},
};
