...
// 그날 실제 한국어해설 경기가 있는 리그만 동적으로 추가하기 위한 매핑.
// misleading hashtag로 노출 깎이는 걸 막기 위해 그날 없는 리그 태그는 절대 넣지 않음.
...
function getDynamicLeagueTags(today: string, max: number): string[] {
  try {
...
export function buildShortsMeta(mm: string, dd: string, today: string) {
  ...
  const fixedHashtags = ["#Shorts", "#한해설", "#한국어해설", "#스포츠중계", "#편성표"];
