...
export const HERO_WEIGHTS = {
  // 30 → 18 (2026-08-05) → 26 (2026-08-07).
  //
  // 18 로 내린 건 "히어로 다양성"이 목적이었고 조회수 근거는 없었다. 실제로 히어로가
  // 코리안리거를 벗어난 첫 두 편(8/06 저녁·8/07 아침, 둘 다 KIA)이 27·6 회로 최근 최저다.
  // 반대로 상위 기록은 전부 코리안리거 MLB 였다(1156·1000·878·749).
  //
  // 다양성은 이제 repeatPenalty 가 따로 맡는다(같은 팀 사흘 연속 차단). 그러니 다양성을
  // 위해 코리안 신호까지 깎을 이유가 없다. 표본 2편이라 확정은 아니고 관찰 중인 가설이다.
  //
  // 값은 7일 시뮬레이션으로 골랐다(2026-08-07 실측):
  //   18 → 08/08 첼시 vs AC밀란(친선) · 08/09 맨시티 내한 · 이후 MLB
  //   26 → 08/08 이정후 · 08/09 맨시티 내한 · 이후 MLB      ← 채택
  //   30 → 08/09 맨시티 내한까지 MLB 에 밀림               ← 내한경기를 잃어 손해
  koreanPlayer: 26,
