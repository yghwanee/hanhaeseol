...
/**
 * 리그·플랫폼 외 허브. `한국어 해설` 은 거의 모든 글에 나오고 네이버에서 CTR 이 가장
 * 높은 쿼리군이라 착지 페이지로 연결할 값어치가 크다.
 *
 * 2026-07-30: `/worldcup` 허브 제거(대회 종료)로 "북중미 월드컵"·"월드컵" 엔티티도
 * 같이 뺐다 — 가리킬 페이지가 없어졌다. 월드컵 글은 이제 이 두 단어로 자동링크되지
 * 않는다(리그·플랫폼·한국어 해설 언급이 있으면 그쪽으로는 계속 링크된다).
 */
