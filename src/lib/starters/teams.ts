...
const MLB_ALIASES: Record<string, string> = {
  // LA 두 팀은 표기가 셋이다 — 네이버("LA다저스"), 편성 축약("LA 다저스"),
  // 편성 풀네임("로스앤젤레스 다저스"). 공백을 지우고 비교하므로 축약형은 위 둘로 덮이지만
  // 풀네임은 별도 키가 필요하다. 없으면 정규화가 통째로 실패해
  // `2026-09-05|PIT-로스앤젤레스 에인절스` 같은 키가 생겨 선발이 영영 안 붙는다(2026-09-03 실측).
  "LA다저스": "LAD",
  "로스앤젤레스다저스": "LAD",
  "LA에인절스": "LAA",
  "로스앤젤레스에인절스": "LAA",
