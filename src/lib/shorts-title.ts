...
// 🔴 주어가 선수냐 팀이냐로 서술어가 갈린다. who 는 매치업("A vs B")이 아니라
// **이름 하나**다(선수명 또는 팀명 하나) — 그래서 팀일 때 "첼시 열립니다"·"첼시 출격"
// 같은 비문이 나온다(2026-08-29 시뮬레이션에서 실제로 나왔다). 팀에는 "경기"를 붙인다.
const start = (c: HookCtx) => (c.isPlayer ? "출격" : "경기 시작");
const appear = (c: HookCtx) => (c.isPlayer ? "나옵니다" : "경기 열립니다");
