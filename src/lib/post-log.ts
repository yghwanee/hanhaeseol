 ...
 *
 * 🔴 그래서 이 기록은 **추가 전용(append-only)** 이다. `updatePostLog` 의 mutate 에서
 * 키를 지워도 여기서 원격 값이 되살아난다(2026-09-02 자체 검증에서 실측: 삭제가
 * 조용히 무시됐다). 의도한 성질이다 — 게시 사실이 실수로 사라지면 그 채널이
 * **다시 올라가서 중복 게시**가 되고, 그게 이 파일이 존재하는 이유 자체다.
 * 기록을 줄이는 유일한 경로는 보관 기간이 지난 것을 버리는 `pruneLog` 다
 * (병합 뒤에 돌기 때문에 그건 정상 동작한다).
 * 특정 키를 진짜로 지워야 하면 Contents API 로 직접 쓸 것.
 */
