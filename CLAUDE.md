...
### 다음 작업 (예정)
- 🔴 **네이버 서치어드바이저 확인 (제일 큰 미지수)** — 유입의 **78%가 네이버**인데 한 번도 안 봤음. 2026-07-20에 판단한 모든 것(팀 페이지 전략 포함)이 **구글 데이터, 즉 전체의 1%**를 근거로 한 것. searchadvisor.naver.com → 웹마스터도구 → 리포트 → 검색 노출/클릭 현황 + 웹 페이지 수집 현황. 여기 열어보면 우선순위가 바뀔 수 있음. **사용자만 뽑을 수 있음.**
