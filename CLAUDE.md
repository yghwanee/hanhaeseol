...
69. 후원 버튼 2단계 UX (2026-07-30) — 사용자 요청으로 **금액 선택 → 앱 선택** 2단계로 변경. 종전엔 금액을 누르면 **토스를 자동으로 열고** 실패하면 카카오로 넘기는 연쇄였는데, 무엇이 열릴지 모른 채 앱이 튀는 게 불친절했다. 이제 2단계에서 **토스(파랑 `#3182F6`) / 카카오페이(노랑 `#FEE500`) 타일 2개**를 보여주고 누른 것만 연다. `Phase`에 `method` 상태 추가(`pick → method → opening → fallback`), 금액·카카오링크를 단계 간에 들고 다녀 되돌아가도 유지된다.
    ...
    - **부수 정리(같은 날)**: 폐기된 `NEXT_PUBLIC_DONATE_KAKAOPAY_URL`·`_AMOUNT` 를 Vercel Production·Preview 에서 제거(둘 다 값이 빈 문자열이었음). `vercel link` 가 `.gitignore` 에 `.env*` 를 추가했는데 추적 중인 env 파일이 없어 안전하고, 계좌를 다루는 지금 유용한 안전장치라 남겼다.

### 다음 작업 (예정)
- 🔴 **후원 버튼 — 코드 완성·배포됨, Vercel 환경변수 설정만 남음(작업67)**. 값 넣기 전까지는 버튼이 **스스로 렌더 안 되므로** 사이트에 아무 변화 없음(실측 확인). 사용자가 Vercel 대시보드에 넣을 것:
  - `NEXT_PUBLIC_DONATE_BANK` (예: `토스뱅크`) · `NEXT_PUBLIC_DONATE_ACCOUNT` (숫자만) — **이 둘이 필수**, 하나라도 없으면 숨김
  - `NEXT_PUBLIC_DONATE_HOLDER` (예금주, 선택)
  - **카카오페이는 티어별로 따로**(선택): `NEXT_PUBLIC_DONATE_KAKAOPAY_1900` / `_4900` / `_9900`. 각각 그 금액으로 만든 카카오페이 송금 링크. 설정한 티어에서만 버튼이 뜬다. **`_URL`+`_AMOUNT` 쌍은 폐기됨**(작업68)
  - 넣은 뒤 **재배포 필요**(`NEXT_PUBLIC_*`는 빌드시 번들에 인라인된다). 채운에도 같은 컴포넌트를 붙일 예정(채운은 private 레포).
