...
90. 🔴 애드핏 슬롯 2개가 서버에서 `NO_AD` 였다 + 노필 자리 접기 (2026-08-14) — "인라인·우측 광고가 안 나온다"는 신고. **우리 코드는 전부 정상이었고 애드핏이 광고를 안 주고 있었다.**
    ...
    - **🔴 해결됨(같은 날) — 원인은 유닛 개별 문제였고 재생성으로 풀렸다.** 대시보드 확인 결과 5개 유닛 전부 `승인`·유형도 코드와 일치해 "유닛 상태" 가설은 기각됐다. 그래서 **우리 코드 변수를 전부 제거하며 밀어붙였다**: 마크업을 애드핏 공식 형태로, SDK 를 공식 `t1.daumcdn.net` 으로, 페이지를 본문 있는 최소 HTML 로, 컨텍스트를 매번 새로(쿠키·캐시 격리), 슬롯을 페이지에 1개만. **그래도 대조군(`한해설_PC`)만 OK 였다.** 요청 파라미터도 27개 전수 대조해 **유닛 ID·랜덤 컨테이너ID 외 전부 동일**(referer 포함)함을 확인 → 우리 쪽에서 차이를 만들 지점이 없다는 게 실증됐다.
      ...
      - **남은 것(사용자)**: 안 쓰는 유닛 대시보드에서 삭제 — 한해설 `DAN-lcALm7uz8M1Y77F7`(옛 인라인 728) · `DAN-CF8kPxnkszA1dow9`(옛 우측 160), 채운 `DAN-4ADAx9fipVujhEAY`(옛 상단 728). `DAN-5jZ75p7iWhBxfPY5`(인라인 728)은 지금 안 쓰지만 되돌릴 여지가 있어 남겨 둬도 된다.
...
- ~~🔴 애드핏 유닛 2개 상태 확인~~ **해결 완료(2026-08-14, 작업90)** — 유닛 재생성/교체로 **세 슬롯 전부 정상 송출** 중. 남은 건 사용자가 대시보드에서 **죽은 옛 유닛 2개 삭제**(`DAN-lcALm7uz8M1Y77F7` · `DAN-CF8kPxnkszA1dow9`)뿐이고 코드는 이미 안 쓴다.
  - **관측할 것**: 애드핏 적립금이 작업86 수준(월 ~500원, 슬롯 1개만 살아 있던 상태)에서 오르는지. 세 슬롯이 다 도니 눈에 띄게 올라야 이 작업이 실제로 먹은 것이다.
  - **인라인 PC 는 지금 300x250** 이다(원래 728x90). 728 로 되돌리고 싶으면 새 728x90 유닛을 만들어 `SLOTS.inline.pc` 만 바꾸면 된다 — 그 자리는 breakout(`min-[800px]:-mx-12`)도 같이 되살려야 한다.
