...
- 🔴 **애드핏 매체 심사 대기 (2026-07-29 설치 완료)** — 광고단위 2개 설치·배포 완료. **애드핏은 스크립트 설치 시점부터 매체 심사를 시작하고, 심사 통과 전에는 광고를 송출하지 않는다**(영업일 1~2일). 그래서 설치 직후 라이브에서 `ins`가 `display:none`인 건 정상이다(실브라우저로 확인 — 헤드리스 필터링 아님). **확인할 곳: 광고관리 → 매체 → 상태(심사 중 / 보류 / 반려).** `광고단위 상태: 정상`은 단위가 살아있다는 뜻일 뿐 매체 심사와 별개다. **보류면 사유를 받아 대응 가능**(반려와 달리 재신청됨). 심사 사유에 "콘텐츠가 부족하거나 사용성이 떨어지는 경우"가 있어 애드센스 4차 거절과 같은 판정 축이다.
  - 광고단위: PC `DAN-STkkoddKPkuh5cwy` 728x90 / 모바일 `DAN-sevERV2qTEmuNFC7` 320x50. 코드는 `src/app/_components/AdfitBanner.tsx` 한 곳.
  - **CSS 미디어쿼리로 둘 다 렌더하면 안 됨** — 감춰진 쪽도 노출이 집계돼 무효 노출이 된다. 뷰포트로 한쪽만 렌더하고 리사이즈에는 반응하지 않는다(재마운트마다 노출이 새로 잡힘).
  - **브레이크포인트 800px**(768 아님). 728 + 좌우 패딩 32 = 760px 이고 데스크톱 스크롤바가 15~17px 을 더 먹어 768px 창에서는 7px 넘친다. **Playwright 기본 뷰포트엔 스크롤바가 없어 테스트로는 안 잡히므로 폭을 직접 계산해야 한다.**
  - **배치는 편성표 목록 끝**(홈 상단 아님). 상단이면 광고 로드가 본문을 밀어 CLS를 깎고, 애드핏 정책 제5.3조(실수 클릭 위치 금지)에도 가깝다. 광고단위명이 "PC/Mobile"이지 위치명이 아니다.
  - 미충전 시 `display:none` 유지 → 대체 광고 **미설정이 맞다**(흰 박스가 안 뜨어 다크 테마에 유리).
  - 정산: 최소 지급액 **5만원**, 매달 지정일에 확정 적립금 전환.
