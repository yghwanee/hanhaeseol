...
90. 🔴 애드핏 슬롯 2개가 서버에서 `NO_AD` 였다 + 노필 자리 접기 (2026-08-14) — "인라인·우측 광고가 안 나온다"는 신고. **우리 코드는 전부 정상이었고 애드핏이 광고를 안 주고 있었다.**
    ...
    - **🔴 해결됨(같은 날) — 원인은 유닛 개별 문제였고 재생성으로 풀렸다.** 대시보드 확인 결과 5개 유닛 전부 `승인`·유형도 코드와 일치해 "유닛 상태" 가설은 기각됐다. 그래서 **우리 코드 변수를 전부 제거하며 밀어붙였다**: 마크업을 애드핏 공식 형태로, SDK 를 공식 `t1.daumcdn.net` 으로, 페이지를 본문 있는 최소 HTML 로, 컨텍스트를 매번 새로(쿠키·캐시 격리), 슬롯을 페이지에 1개만. **그래도 대조군(`한해설_PC`)만 OK 였다.** 요청 파라미터도 27개 전수 대조해 **유닛 ID·랜덤 컨테이너ID 외 전부 동일**(referer 포함)함을 확인 → 우리 쪽에서 차이를 만들 지점이 없다는 게 실증됐다.
      ...
      - **🔴 진짜 규칙은 이거였다 — 한 매체에 같은 규격 유닛이 둘이면 하나만 채워진다.** 인라인을 728x90 새 유닛(`DAN-5jZ75p7iWhBxfPY5`)으로 만들어 꽂았더니 **그것도 8회 연속 `NO_AD`** 였다. 상단이 이미 728x90 을 쓰고 있기 때문이다. 데이터가 전부 이 규칙과 맞는다:

        | 규격 | 매체 내 유닛 수 | 결과 |
        |---|---|---|
        | 728x90 | **2개**(상단·인라인) | 상단만 ✅ / 인라인은 **옛 유닛도 새 유닛도** ❌ |
        | 320x50 · 300x250 · 160x600 | 각 1개 | 전부 ✅ |

        즉 우측 160x600 이 재생성으로 살아난 건 "재생성" 덕이 아니라 **그 규격이 매체에서 유일**하기 때문일 수 있다(옛 우측 유닛이 왜 죽었는지는 여전히 미확인 — 재생성으로 해결됐으니 더 파지 않는다).
      - **최종 구성**: 상단 728x90 `DAN-STkkoddKPkuh5cwy` / 모바일 상단 320x50 `DAN-sevERV2qTEmuNFC7` / **인라인 PC·모바일 공용 300x250 `DAN-kJ3thvrkQ3G6WyUG`** / 우측 160x600 `DAN-1fL2TxxOBIE9A7nT`. **인라인 PC 는 300x250 이라 breakout(`-mx-12`) 이 없다.**
      - **🔴 인라인을 728x90 으로 되돌리고 싶으면 상단 규격을 먼저 바꿔야 한다.** 728 유닛을 하나 더 만들어 꽂으면 같은 이유로 또 죽는다 — 이미 두 번 겪었다.
      - **교훈 둘**: ①애드핏 유닛은 `승인` 상태이고 규격도 맞는데 서버가 `NO_AD` 만 돌려주는 경우가 있다. 대시보드로는 구분이 안 된다. ②**슬롯을 새로 만들 때는 매체에 없는 규격을 고를 것.** 판별은 `serv.ds.kakao.com/sdk/banner` 응답의 `status` 로 한다.
      - **전 페이지 최종 검증(PC 1440 · 모바일 390, 13페이지 26조합)**: 광고 있는 페이지 전부 `ins` 개수·유닛 정확, 유닛 중복 0, SDK 스크립트 페이지당 1개, 가로 스크롤 0, HTTP 200. `/commentary`·`/guide`·`/team` 은 **상단 슬롯이 없어** PC 에선 우측만, 모바일에선 광고가 없다(설계대로 — 넣으려면 별도 규격 유닛이 필요하다).
      - **남은 것(사용자)**: 안 쓰는 유닛 3개 대시보드에서 삭제 — `DAN-lcALm7uz8M1Y77F7`(옛 인라인 728) · `DAN-CF8kPxnkszA1dow9`(옛 우측 160) · `DAN-5jZ75p7iWhBxfPY5`(새로 만들었지만 728 중복이라 못 쓰는 것). 코드는 셋 다 안 쓴다.
