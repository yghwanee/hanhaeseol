...
- 🔴 **관측 (작업106, 2026-09-02)** — 내일부터 확인할 것 셋. ①텔레그램이 **하루 2통**(아침 보고·저녁 보고)으로 줄었는지 ②`instagram*.yml` 실행이 같은 사이클에 여러 번 성공하지 않는지(`gh run list` 로 dispatch 가 몰리는 구간이 사라져야 한다) ③저녁분이 다시 **KST 18~19시**에 나가는지(9/01 은 13:59 에 나갔다). 뭔가 빠지면 `data/post-log.json` 을 먼저 볼 것 — 게시 사실의 단일 진실 원본이다.
