...
- 🔴 **애드핏 유닛 2개 상태 확인 (사용자 몫, 2026-08-14 작업90에서 넘김)** — 카카오 애드핏 → 광고단위에서 **`한해설_PC_인라인`(`DAN-lcALm7uz8M1Y77F7`)** 과 **`한해설_PC_오른쪽`(`DAN-CF8kPxnkszA1dow9`)** 의 **상태**를 볼 것(`운영중` / `중지` / `심사중` / `반려`).
  - `중지`·`심사중`·`반려` → 되살리면 끝. `운영중`인데도 노필이면 수요 부족이라 **우측 160x600 은 300x250 새 유닛으로 교체**한다(같은 유닛의 규격만 바꾸는 건 안 통한다 — 작업90 실험).
  - 노필 자리는 이제 접히므로 **화면상으로는 아무 문제가 안 보인다.** 즉 이 항목은 눈으로 알아챌 수 없고 **수익에만 조용히 반영된다** — 잊지 말 것.
  - 재확인 방법은 작업90 의 headed 브라우저 + `serv.ds.kakao.com/sdk/banner` 응답 읽기.
