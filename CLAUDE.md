...
97. 🔴 선수 로스터가 코드에 박혀 있어 이적을 못 따라갔다 + 흐름 서술이 전적과 반대였다 (2026-08-23) — 사용자 신고 2건. 둘 다 "부탁"으로 관리하던 것을 **데이터·검사로** 바꿨다.
    ...
    - **남은 것(사람 판단 필요)**: 커버·쇼츠에 쓰는 `이강인 출전`·`출격` 표현은 **소속팀이 경기한다는 뜻이지 선발 출전 보장이 아니다.** 라인업은 경기 1시간 전에나 나오므로 전날 저녁 게시 시점에는 알 수 없다. 표현을 `소속팀 경기`로 낮추면 정확해지지만 후킹이 약해진다 — 지금은 그대로 뒀다.
