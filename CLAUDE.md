...
83. 🔴 히어로 선정 개편 + 커버 후킹 도입 (2026-08-05, 커밋 `16020a36`) — "커버에 후킹이 필요하다"는 요청에서 시작했는데 코드를 뜯어보니 결함이 두 층이었다.
    ...
    - **두 슬롯이 이제 같은 레이아웃(풀이미지 + 하단 텍스트)을 쓴다.** 구분은 **액센트 색**(아침 앰버 `#ffb02e` / 저녁 라임 `#8fff3d`) + 문구 구조(아침 작은줄→큰줄 / 저녁 큰줄→설명줄) + 배경 사진 + 영상 경로(v1/v2). 6개 요소 중 5개가 다르다. **`HHS_LEGACY_MORNING` 의 뜻이 바뀌었다**: "V7 카드를 쓰는가" → "릴스 v1 첫 프레임용 9:16 을 하나 더 만드는가". `renderHookV7` 은 파일에 남아 있으나 게시 경로에서 빠졌다.
...
- ~~**유튜브 중복 게시 정리**~~ **대응 완료(2026-08-05, 작업82+83)** — 실제로 배포가 눌렸고(피드 조회 3) 슬롯별 제목·커버·첫 프레임 분리(82) + 커버 후킹·액센트 색 분리(83)로 대응. **🔴 남은 건 관찰인데 두 변경이 섞였다** — 회복돼도 뭐가 먹었는지 못 가른다. 1~2주 뒤 Shorts 피드 조회 확인, 안 되면 다음 레버는 영상 독창성(TTS·실사컷) 또는 하루 1회 감축.
- **커버 후킹 첫 실전 확인 (작업83)** — 오늘 저녁 워크플로(KST 21시대)가 첫 실행. 텔레그램 보고에서 5채널(캐러셀·릴스·스토리·유튜브·틱톡) 전부 ✅ 인지 볼 것. 실패분만 재실행 = `gh workflow run instagram.yml -f only=<채널>`.
