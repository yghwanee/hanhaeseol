...
88. 🔴 네이버가 홈 메타를 통째로 버리고 있었다 + 중복 호스트 3종 + 브랜드 신호 (2026-08-13, 커밋 `74a4c2e4`) — "`한해설` 로 검색하면 홈이 아니라 서브페이지가 뜬다"는 신고에서 시작. 원인이 세 층이었고 **제일 큰 건 우리 코드의 부작용**이었다.
    ...
    - **🔴 중복 URL 3종** (동료 `lupin-lab.co.kr` SEO 문서가 "가장 효과 컸던 것 2위"로 꼽은 항목이고, 그쪽 지적 그대로 *canonical 은 제안이지 명령이 아니다*):
      ...
      - `www.haeseol.com` 이 **307(임시)** → 301 규칙을 `next.config.mjs` 에 넣었지만 **Vercel 도메인 리다이렉트가 엣지에서 먼저 먹어 여전히 307**이다. 🔴 **대시보드 Settings › Domains › www.haeseol.com 에서 308 로 바꿔야 한다(코드로 못 고침).**
    ...
    - **관측(1~2주)**: ①네이버 간단체크를 다시 돌려 `사이트 제목`·`사이트 설명`·`Open Graph` 가 ✓ 로 바뀌는지 — **이게 오늘 작업의 판정 기준이다** ②`한해설` 검색에 홈이 뜨는지 ③색인 수 880 기준 변화.
