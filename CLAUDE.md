...
- **종목별 페이지 (근거는 있는데 크기가 작다)** — `야구 편성표` 11.7% · `오늘 야구 해설` 19.4% 가 **전용 페이지 없이도** 나온다. 라우트가 `league·platform·standings·team·match·commentary·guide` 뿐이라 종목별이 통째로 비어 있다. 데이터는 있다(7일치 야구 113·축구 43). 🔴 **농구 2·배구 0 이라 그대로 만들면 빈 페이지가 생긴다**(팀 페이지 138개 교훈) → 야구·축구만 만들고 경기 수 게이트로 자동 편입. 다만 관측 노출이 354뿐이라 **작업85 CTR 판정 후에 착수 여부를 정한다.**
