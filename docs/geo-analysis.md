...
- 저자 정보는 **이미 있다** — `/guide`는 `NewsArticle` + `author: Organization` + `datePublished`/`dateModified`를 갖고 있고 본문에도 날짜가 보인다. 초안 대부분이 자동 생성이므로 개인 저자를 붙이는 건 사실과 다르다(Organization 귀속이 맞다). 이 항목은 조치 불필요.
...
| 2 | 원본 데이터 발표 — 예: "플랫폼별 한국어 해설 비율" 월간 집계 | 우리만 가진 데이터, 인용 유발력 높음 |
| 3 | `/guide` 정기 갱신 프로그램(6개월 방치 방지) | 운영 |
| 4 | 비교표 확대(플랫폼 × 리그 매트릭스) | 코드 (M) |

2번이 특히 우리 강점이다. 10개 플랫폼의 한국어 해설 여부를 매일 수집하는 곳은 우리뿐이고, 그 집계는 다른 데 없는 원본 데이터다.
